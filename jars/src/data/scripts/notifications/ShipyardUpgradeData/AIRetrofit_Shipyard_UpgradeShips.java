...
public class AIRetrofit_Shipyard_UpgradeShips {
    public int type;
    public ArrayList<AIRetrofit_Shipyard_UpgradeShip> ships = new ArrayList<>();
    public AIRetrofit_Shipyard_UpgradeShips(int type){
        this.type = type;
    }
    ...
    public void addShip(FleetMemberAPI ship, int size,boolean bonus){
        ships.add(new AIRetrofit_Shipyard_UpgradeShip(ship,size,bonus));
    }
    public float[] display(TooltipMakerAPI info, int type, boolean playerOwned){
        ...
        for(int a = 0; a < ships.size(); a++){
            AIRetrofit_Shipyard_UpgradeShip ship = ships.get(a);
