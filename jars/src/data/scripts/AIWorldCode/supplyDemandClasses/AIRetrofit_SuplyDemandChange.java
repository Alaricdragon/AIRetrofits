...
public class AIRetrofit_SuplyDemandChange {
    ...
    public AIRetrofit_SuplyDemandChange(String nametemp,boolean supplytemp){
        name = nametemp;
        supply = supplytemp;
    }
    ...
    public void undo(Industry industry,String ID){
        ...
        for(int a = 0; a < s.size(); a++){
            s.get(a).getQuantity().unmodifyMult(ID);
            s.get(a).getQuantity().unmodifyFlat(ID);
            industry.getSupply(ID).getQuantity().unmodifyFlat(ID);
        }
    }
