...
public class AIRetrofit_MarketGrowthMods implements MarketImmigrationModifier {
    ...
    static String condition = AIRetrofits_Constants_3.Market_Condition;//"AIRetrofit_AIPop";
    ...
    private static void modifyGrowth(MarketAPI market){
        AIRetrofit_Log.loging("hiding notified market modifiers for market " + market.getName(),logsClass, AIRetrofits_Constants_3.Market_EnableLogs);
        PopulationComposition incoming = market.getIncoming();
        String[] activeGrowth = AIRetrofits_Constants_3.Market_WhiteListedGrowthMods;
        int a = 0;
        while(incoming.getWeight().getFlatMods().keySet().size() != 0 && a < incoming.getWeight().getFlatMods().keySet().size()){
            ...
            if(out) {
                AIRetrofit_Log.loging("hiding modifier named " + incoming.getWeight().getFlatMods().keySet().toArray()[a].toString(),logsClass, AIRetrofits_Constants_3.Market_EnableLogs);
                incoming.getWeight().unmodify(incoming.getWeight().getFlatMods().keySet().toArray()[a].toString());
            }
        }
    }
    private static final String T1FactoryName = AIRetrofits_Constants_3.Market_GrowthMod_LocalRobotFactorys;
    private static final String T1FactoryDescription = AIRetrofits_Constants_3.Market_GrowthDescription_LocalRobotFactorys;
    private static final String T2FactoryName = AIRetrofits_Constants_3.Market_GrowthMod_FactionWideRobotFactorys;
    private static final String T2FactoryDescription = AIRetrofits_Constants_3.Market_GrowthDescription_FactionWideRobotFactorys;
    ...
    private static void addRobotFactory(PopulationComposition incoming,MarketAPI market){
        AIRetrofit_Log.loging("adding T1 and T2 robot factory bonuses",logsClass, AIRetrofits_Constants_3.Market_EnableLogs);
    ...
    static String hazzardPayName = AIRetrofits_Constants_3.Market_GrowthMod_hazzardPayName;
    static String hazzardPayDescription = AIRetrofits_Constants_3.Market_GrowthDescription_hazzardPay;
    static float hazzardGrowthPerSize = AIRetrofits_Constants_3.Market_GrowthMod_hazzardGrowthPerSize;
    ...
    private static void addHazardPay(PopulationComposition incoming,MarketAPI market){
        if(market.isImmigrationIncentivesOn()){
            ...
            AIRetrofit_Log.loging("adding hazardPay boost to market...",logsClass, AIRetrofits_Constants_3.Market_EnableLogs);
        }
    }
